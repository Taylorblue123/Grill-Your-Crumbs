...
/* ============================================================
   ...
   **不连 token 也能用**：公开仓贴地址就行，这是成本最低的一条路。旁边的
   GitHubBox 用 PAT 换来私有仓和批量勾选——两条路并存而不是后者取代前者，
   因为让只想连一个公开仓的人先去开一个 token，是把门槛加在不需要它的人身上。
   （连了 token 之后这条路也会自动带上它，于是贴私有仓地址同样能连。）
  ...
  return (
    <form className="repo" onSubmit={submit}>
      ...
      <small className="repo-hint">
        ...
        {' '}
        私有仓、或者想一次挑好几个？<b>用下面的「连接 GitHub」</b>。
      </small>
        ...
        <div className="repo-msg err" role="alert">
          ...
          {error.fallback && (
            ...
            <span className="repo-fallback">
              这是个私有仓的话，<b>先连一个 GitHub token</b>（就在下面）就能拉。
              等不了限流、或者不想连 token？<b>把仓库的 README 当文件上传</b>
              也行——上面的上传框收 .md / .txt，效果和连仓是同一种料。
            </span>
          )}
        </div>
      )}
